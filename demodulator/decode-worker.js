...
function Decoder() {
   ...
   */
  function process(buffer, inStereo, freqOffset, sampleRate) {
    IN_RATE = sampleRate;
    var data = {};
   ...
   */
  function setMode(mode, sampleRate) {
    switch (mode.modulation) {
      case 'AM':
        demodulator = new Demodulator_AM(sampleRate, OUT_RATE, mode.bandwidth);
        break;
      case 'USB':
        demodulator = new Demodulator_SSB(sampleRate, OUT_RATE, mode.bandwidth, true);
        break;
      case 'LSB':
        demodulator = new Demodulator_SSB(sampleRate, OUT_RATE, mode.bandwidth, false);
        break;
      case 'NBFM':
        demodulator = new Demodulator_NBFM(sampleRate, OUT_RATE, mode.maxF);
        break;
      default:
...
onmessage = function (event) {
  switch (event.data[0]) {
    case 1:
      decoder.setMode(event.data[1], event.data[2]);
      break;
    default:
      decoder.process(event.data[1], event.data[2], event.data[3], event.data[4], event.data[5]);
      break;
  }
};
