 ...
 * @constructor
 */
module.exports.Demodulator_WBFM =  function(inRate, outRate) {
  ...
  var demodulator = dsp.FMDemodulator(inRate, INTER_RATE, MAX_F, FILTER, 51);
  var filterCoefs = dsp.getLowPassFIRCoeffs(INTER_RATE, 10000, 41);
  var monoSampler = dsp.Downsampler(INTER_RATE, outRate, filterCoefs);
  var stereoSampler = dsp.Downsampler(INTER_RATE, outRate, filterCoefs);
  var stereoSeparator = dsp.StereoSeparator(INTER_RATE, PILOT_FREQ);
  var leftDeemph = dsp.Deemphasizer(outRate, DEEMPH_TC);
  var rightDeemph = dsp.Deemphasizer(outRate, DEEMPH_TC);
